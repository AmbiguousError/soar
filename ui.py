...
def draw_dial(surface, center_x, center_y, radius, hand_angle_degrees, hand_color, dial_color=config.PASTEL_GRAY, border_color=config.PASTEL_TEXT_COLOR_HUD, label="N"):
...
def draw_start_screen_content(surface):
    ...
    draw_text(surface, "- Dogfight: Shoot down enemy waves!", info_font_size, config.SCREEN_WIDTH // 2, info_y,
              config.PASTEL_LIGHT_GRAY, font_name=config.HUD_FONT_NAME, center=True)
    info_y += line_spacing
    draw_text(surface, "- Delivery: Take off & land at designated runways.", info_font_size, config.SCREEN_WIDTH // 2, info_y,
              config.PASTEL_LIGHT_GRAY, font_name=config.HUD_FONT_NAME, center=True)
    info_y += line_spacing * 1.5
...
def draw_mode_select_screen(surface, selected_option_idx):
    surface.fill(config.PASTEL_DARK_GRAY)
    draw_text(surface, "Select Mode", 56, config.SCREEN_WIDTH // 2, config.SCREEN_HEIGHT // 4 - 40, config.PASTEL_GOLD, font_name=config.HUD_FONT_NAME, center=True, shadow=True, shadow_color=config.PASTEL_BLACK)
    modes_display = [
    ...
    for i, (name, desc, mode_const) in enumerate(modes_display):
        ...
        y_pos = option_base_y + i * 70
        draw_text(surface, name, 44, config.SCREEN_WIDTH // 2, y_pos, color, font_name=config.HUD_FONT_NAME, center=True, shadow=True, shadow_color=config.PASTEL_BLACK)
        draw_text(surface, desc, 20, config.SCREEN_WIDTH // 2, y_pos + 30, color, font_name=config.HUD_FONT_NAME, center=True)
...
def draw_game_over_screen_content(surface, final_player_height_or_score, level_or_round_reached, high_scores_data, current_game_mode_data, total_laps_data=0, dogfight_round_reached=0, deliveries_completed=0):
    surface.fill(config.PASTEL_DARK_GRAY)
    ...
    if current_game_mode_data == config.MODE_FREE_FLY:
        draw_text(surface, f"Reached Level: {level_or_round_reached}", config.HUD_FONT_SIZE_LARGE, config.SCREEN_WIDTH // 2, y_offset, config.PASTEL_WHITE, font_name=config.HUD_FONT_NAME, center=True); y_offset += line_height
        draw_text(surface, f"Max Altitude This Session: {int(high_scores_data['max_altitude_free_fly'])}m", config.HUD_FONT_SIZE_NORMAL, config.SCREEN_WIDTH // 2, y_offset, config.PASTEL_WHITE, font_name=config.HUD_FONT_NAME, center=True); y_offset += line_height
        draw_text(surface, f"Longest Flight This Session: {high_scores_data['longest_flight_time_free_fly']:.1f}s", config.HUD_FONT_SIZE_NORMAL, config.SCREEN_WIDTH // 2, y_offset, config.PASTEL_WHITE, font_name=config.HUD_FONT_NAME, center=True); y_offset += line_height
        draw_text(surface, f"Final Height: {int(final_player_height_or_score)}m", config.HUD_FONT_SIZE_NORMAL, config.SCREEN_WIDTH // 2, y_offset, config.PASTEL_WHITE, font_name=config.HUD_FONT_NAME, center=True); y_offset += line_height
    elif current_game_mode_data == config.MODE_RACE:
        if total_laps_data in high_scores_data["best_total_race_times"]:
             draw_text(surface, f"Best Race ({total_laps_data} Laps): {high_scores_data['best_total_race_times'][total_laps_data]:.1f}s", config.HUD_FONT_SIZE_NORMAL, config.SCREEN_WIDTH // 2, y_offset, config.PASTEL_WHITE, font_name=config.HUD_FONT_NAME, center=True); y_offset += line_height
        if high_scores_data["best_lap_time_race"] != float('inf'):
            draw_text(surface, f"Best Lap: {high_scores_data['best_lap_time_race']:.1f}s", config.HUD_FONT_SIZE_NORMAL, config.SCREEN_WIDTH // 2, y_offset, config.PASTEL_WHITE, font_name=config.HUD_FONT_NAME, center=True); y_offset += line_height
