...
def find_suitable_runway_location(existing_locations, map_offset_x, map_offset_y, cache, required_min_distance_apart): # Added required_min_distance_apart
    """Finds a random suitable location for a runway, respecting a minimum distance from existing locations."""
    for _ in range(config.RUNWAY_MAX_PLACEMENT_ATTEMPTS):
        ...
        if land_type in config.RUNWAY_SUITABLE_LAND_TYPES:
            for loc in existing_locations:
                if math.hypot(rx - loc[0], ry - loc[1]) < required_min_distance_apart: # Use passed-in dynamic distance
                    too_close = True
                    break
            if not too_close:
                return rx, ry
...
def setup_delivery_mission():
    ...
    # Calculate dynamic minimum distance for runway separation
    # current_level is 1 for the first mission. No scaling for the first mission.
    # For the second mission (current_level=2, meaning 1 previous success), apply scaling once.
    num_previous_successes = current_level - 1
    if num_previous_successes < 0: num_previous_successes = 0 # Should not happen if current_level starts at 1

    dynamic_min_distance = config.RUNWAY_MIN_DISTANCE_APART * (1 + (num_previous_successes * config.DELIVERY_MIN_DISTANCE_INCREASE_FACTOR))
    # Ensure there's also some absolute minimum, in case the base is very small or factor is negative (though not intended)
    dynamic_min_distance = max(dynamic_min_distance, config.RUNWAY_MIN_DISTANCE_APART / 2)


    # Find start runway location
    # No existing locations for the first runway, so min_distance check against others is not applicable here.
    # However, the second runway needs to be at least dynamic_min_distance from the first.
    start_loc = find_suitable_runway_location([], current_map_offset_x, current_map_offset_y, tile_type_cache, 0) # Pass 0 as min_dist for the first runway
    if not start_loc:
    ...
    # Find destination runway location, respecting the dynamic minimum distance from the start runway
    dest_loc = find_suitable_runway_location([start_loc], current_map_offset_x, current_map_offset_y, tile_type_cache, dynamic_min_distance)
    if not dest_loc:
        print(f"Error: Could not place destination runway for delivery mission (tried min_dist: {dynamic_min_distance}).")
        # Attempt with base minimum distance as a fallback if scaled one fails too often.
        ...
        if not dest_loc:
            print("Error: Fallback runway placement also failed.")
            reset_to_main_menu() # Fallback
            return
        else:
            print("Warning: Used fallback base minimum distance for destination runway.")
...
def start_new_level(level_param, continue_map_from_race=False):
    ...
    # wingman_was_actually_unlocked_this_turn is reset in setup_delivery_mission or before free fly unlock check.
    # No, it should be reset here too for other modes or if setup_delivery_mission isn't called immediately.
    wingman_was_actually_unlocked_this_turn = False
    ...
    if not continue_map_from_race: 
        ...
        # The seed for river parameters uses current_level, which for Delivery mode, is the mission number.
        # This means river patterns will change with each delivery mission, which is fine.
    ...
    if config.current_game_mode == config.MODE_FREE_FLY:
        ...
        current_level = level_param # This 'level_param' is the mission number for delivery.
        setup_delivery_mission() # This sets game_state, player position, and calculates dynamic runway distance.
        current_thermal_spawn_rate = config.BASE_THERMAL_SPAWN_RATE
        if config.game_difficulty == config.DIFFICULTY_NOOB: current_thermal_spawn_rate = max(20, current_thermal_spawn_rate // 2)
        elif config.game_difficulty == config.DIFFICULTY_EASY: current_thermal_spawn_rate = max(30, int(current_thermal_spawn_rate * 0.75))
        thermal_spawn_timer = 0
...
def update_game_logic(keys):
    ...
    if game_state == config.STATE_RACE_PLAYING:
        ...
        if game_state == config.STATE_DELIVERY_PLAYING:
            ...
            # Reset flag before each potential completion check within a frame (though usually only one completion per frame)
            wingman_was_actually_unlocked_this_turn = False 
    ...
    if game_state == config.STATE_PLAYING_FREE_FLY and player.height >= config.TARGET_HEIGHT_PER_LEVEL * current_level: 
        ...
        if unlocked_wingmen_count < config.MAX_WINGMEN: 
            unlocked_wingmen_count += 1
            # wingman_was_actually_unlocked_this_turn = True # Not directly used by UI for this screen type
        spawn_wingmen() 
