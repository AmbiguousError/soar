...
# Gravity and Lift
GRAVITY_BASE_PULL = 0.18
LIFT_PER_SPEED_UNIT = 0.03
MINIMUM_SINK_RATE = 0.02
...
# Thermals
THERMAL_SPAWN_BASE_RATE = 90 # Slightly less frequent to avoid clutter
THERMAL_LIFESPAN = 800 # Frames, slightly longer
MIN_THERMAL_RADIUS = 20
MAX_THERMAL_RADIUS = 50
BASE_THERMAL_LIFT_POWER = 0.3 # Lift units per frame at nominal speed (before player speed scaling)
...
# Wind
WIND_SPEED_X = -0.5 # Gentle wind from right to left
WIND_SPEED_Y = 0.05 # Slight upward drift

# Clouds
NUM_FOREGROUND_CLOUDS = 8
MIN_CLOUD_SPEED_FACTOR = 1.5 # How much faster clouds move relative to base wind
...
CLOUD_COLOR = (220, 220, 240) # Very light grayish blue
...
THERMAL_COLOR_PRIMARY_TUPLE = (255, 150, 150) # Base RGB for thermals
THERMAL_COLOR_ACCENT_TUPLE = (255, 255, 255)  # Base RGB for thermal accents
...
class Glider(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.current_thermal_lift_bonus = 0 # Lift from thermals this frame
    ...
    def reset(self):
        ...
        self.current_thermal_lift_bonus = 0

    def update(self, keys):
        self.current_thermal_lift_bonus = 0 # Reset for this frame
        ...
        # Apply natural sink/lift only if not getting significant lift from thermals
        # The thermal lift is added directly to self.height in apply_lift_from_thermal
        if self.current_thermal_lift_bonus <= natural_height_change: # If thermal lift isn't overcoming sink
        ...
        # Screen Wrap
        margin = max(self.rect.width, self.rect.height) / 2 # Ensure whole sprite is off before wrap
    ...
    def apply_lift_from_thermal(self, thermal_lift_power_at_nominal_speed):
        ...
        self.current_thermal_lift_bonus += lift_this_frame # Track for this frame
...
class Thermal(pygame.sprite.Sprite):
    def __init__(self, center_pos):
        ...
        # Lift power could also be tied to radius, e.g., bigger thermals are stronger
    ...
    def update_visuals(self):
        # Pulsing animation based on lifespan and time
        pulse_alpha_factor = (math.sin(pygame.time.get_ticks() * 0.005 + self.creation_time * 0.01) * 0.3 + 0.7)
        # Fade out over lifespan
        age_factor = max(0, self.lifespan / THERMAL_LIFESPAN) # 1 when new, 0 when dying
        ...
        # Pulsing radius for visual effect (doesn't change collision radius)
...
class ForegroundCloud(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # Draw a few overlapping ellipses to form a cloud shape
        num_puffs = random.randint(3, 6)
        for _ in range(num_puffs):
            puff_w = random.randint(int(self.width*0.3), int(self.width*0.7))
            puff_h = random.randint(int(self.height*0.4), int(self.height*0.8))
        ...
        # Initial position (spawn off-screen based on wind direction)
        if self.dx < 0: # Moving left
            start_x = SCREEN_WIDTH + self.width / 2 + random.randint(0, 100)
        else: # Moving right (or static if dx=0)
            start_x = -self.width / 2 - random.randint(0, 100)
        
        if self.dy < 0: # Moving up
            start_y = SCREEN_HEIGHT + self.height/2 + random.randint(0,50)
        elif self.dy > 0: # Moving down
            start_y = -self.height/2 - random.randint(0,50)
        else: # No vertical wind component for initial placement
            start_y = random.randint(0, SCREEN_HEIGHT - self.height)
    ...
    def update(self):
        ...
        if self.dx < 0 and self.rect.right < -200: self.kill() # Moving left
        elif self.dx > 0 and self.rect.left > SCREEN_WIDTH + 200: self.kill() # Moving right
        if self.dy < 0 and self.rect.bottom < -200 : self.kill() # Moving up
        elif self.dy > 0 and self.rect.top > SCREEN_HEIGHT + 200 : self.kill() # Moving down
...
pygame.display.set_caption("Pastel Glider - Rideable Thermals & Clouds")
clock = pygame.time.Clock()

# --- Game Objects & Variables ---
player = Glider()
all_sprites = pygame.sprite.Group() # Player, Thermals
...
def reset_game_state():
    ...
    for _ in range(NUM_FOREGROUND_CLOUDS): # Initial cloud population
        cloud = ForegroundCloud()
        foreground_clouds_group.add(cloud)
        # all_sprites.add(cloud) # Clouds are drawn separately for layering
    thermal_spawn_timer = 0; final_score = 0
...
def draw_start_screen_content(surface):
    ...
    draw_text(surface, "UP/DOWN: Speed | LEFT/RIGHT: Bank", 28, SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 70, WHITE, center=True)
    draw_text(surface, "Ride inside thermals to gain height.", 28, SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 30, WHITE, center=True)
    draw_text(surface, "Slower speed in thermals = more lift.", 28, SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 10, WHITE, center=True)
    draw_text(surface, "Higher speed reduces sink rate.", 28, SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 50, WHITE, center=True)
    draw_text(surface, "Don't hit height zero!", 28, SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 90, WHITE, center=True)
    draw_text(surface, "Press ENTER to Start", 36, SCREEN_WIDTH // 2, SCREEN_HEIGHT * 3 // 4 + 20, LIGHT_GRAY, center=True)
    draw_text(surface, "ESC during game for Menu", 22, SCREEN_WIDTH // 2, SCREEN_HEIGHT * 3 // 4 + 70, GRAY, center=True)
...
while running:
    ...
    if game_state == STATE_PLAYING:
        ...
        # Replenish clouds if needed
        if len(foreground_clouds_group) < NUM_FOREGROUND_CLOUDS:
            cloud = ForegroundCloud()
            foreground_clouds_group.add(cloud)
            # all_sprites.add(cloud) # Not adding to all_sprites
        ...
        # Glider-Thermal Continuous Lift Check (Circular Collision)
        player_pos = pygame.math.Vector2(player.rect.center)
        for thermal in thermals_group:
            distance = player_pos.distance_to(thermal.pos)
            # Check if player's approximate center is within thermal's radius
            if distance < thermal.radius + player.radius * 0.5: # player.radius is an approximation
                player.apply_lift_from_thermal(thermal.lift_power)
                # Visual cue for being in thermal (optional)
                # pygame.draw.line(screen, WHITE, player.rect.center, thermal.rect.center, 1)
    ...
    if game_state == STATE_START_SCREEN:
        ...
        foreground_clouds_group.draw(screen) # Draw clouds on top

        # UI Text (ensure it's on top of clouds)
        ...
        wind_display_text = f"Wind: <{WIND_SPEED_X*10:.0f}, {WIND_SPEED_Y*10:.0f}>" # Scaled for display
