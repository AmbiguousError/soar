...
class Minimap:
    ...
    def draw(self, surface, player_glider, ai_gliders_list, course_markers): 
        ...
        for i, marker in enumerate(course_markers):
            ...
            if i == player_glider.current_target_marker_index and marker.number == 1: 
                color = PASTEL_GOLD
    ...
    if game_state == STATE_START_SCREEN:
        ...
        if current_game_mode == MODE_FREE_FLY:
            ...
            if race_course_markers and player.current_target_marker_index < len(race_course_markers) and game_state == STATE_RACE_PLAYING :
                ...
                p1 = (tip_x, tip_y)
                p2 = (tip_x - arrow_len * 0.7 * math.cos(angle_to_marker_screen + math.pi / 4), 
                      tip_y - arrow_len * 0.7 * math.sin(angle_to_marker_screen + math.pi / 4))
                p3 = (tip_x - arrow_len * 0.7 * math.cos(angle_to_marker_screen - math.pi / 4),
                      tip_y - arrow_len * 0.7 * math.sin(angle_to_marker_screen - math.pi / 4))
