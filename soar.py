...
# --- Constants ---
SCREEN_WIDTH = 1200 # Updated
SCREEN_HEIGHT = 900 # Updated
HUD_HEIGHT = 100
...
MINIMUM_SINK_RATE = 0.04 # Max sink rate in normal flight (not stall)
...
GREEN = (0, 200, 0); DARK_GREEN = (0, 100, 0); GOLD = (255, 215, 0)
BLUE = (0,0,255)
...
LAND_TYPE_COLORS = {
    LandType.WATER_DEEP: COLOR_WATER_DEEP, LandType.WATER_SHALLOW: COLOR_WATER_SHALLOW,
    LandType.PLAINS: COLOR_PLAINS, LandType.GRASSLAND: COLOR_GRASSLAND,
    LandType.FOREST_TEMPERATE: COLOR_FOREST_TEMPERATE, LandType.FOREST_DENSE: COLOR_FOREST_DENSE,
    LandType.MOUNTAIN_BASE: COLOR_MOUNTAIN_BASE, LandType.MOUNTAIN_PEAK: COLOR_MOUNTAIN_PEAK,
...
class Glider(pygame.sprite.Sprite):
    def __init__(self):
        ...
        pygame.draw.rect(self.original_image, GLIDER_BODY_COLOR, (0, fuselage_y_top, self.fuselage_length, self.fuselage_thickness))
        wing_leading_edge_x_from_tail = self.fuselage_length * 0.65
        pygame.draw.rect(self.original_image, GLIDER_WING_COLOR, (wing_leading_edge_x_from_tail, 0, self.wing_chord, self.wing_span))
        tail_plane_y_top = (canvas_height - self.tail_plane_span) / 2
        pygame.draw.rect(self.original_image, GLIDER_WING_COLOR, (0, tail_plane_y_top, self.tail_plane_chord, self.tail_plane_span))
...
NUM_MAJOR_RIVERS = 3 
MAJOR_RIVERS_PARAMS = []
_river_param_random = random.Random(54321) 
...
def get_seeded_random_value(tile_x, tile_y, scale, p_pair):
    scaled_x = math.floor(tile_x / scale); scaled_y = math.floor(tile_y / scale)
...
def draw_text(surface, text, size, x, y, color=WHITE, font_name=None, center=False, antialias=True, shadow=False, shadow_color=DARK_GRAY, shadow_offset=(1,1)):
...
def draw_start_screen_content(surface):
    ...
    draw_text(surface, "Pastel Glider", 72, SCREEN_WIDTH//2, SCREEN_HEIGHT//4 - 20, COLOR_PLAINS, center=True, shadow=True)
    draw_text(surface, "Press ENTER to Select Difficulty", 30, SCREEN_WIDTH//2, SCREEN_HEIGHT//2 + 60, LIGHT_GRAY, center=True, shadow=True)
    draw_text(surface, "UP/DOWN: Speed | L/R: Bank", 20, SCREEN_WIDTH//2, SCREEN_HEIGHT*3//4, WHITE, center=True)
    draw_text(surface, "Explore the skies, use thermals to gain height!", 20, SCREEN_WIDTH//2, SCREEN_HEIGHT*3//4 + 30, WHITE, center=True)
...
def draw_game_over_screen_content(surface, score, level):
    surface.fill(DARK_GRAY); draw_text(surface, "GAME OVER", 72, SCREEN_WIDTH//2, SCREEN_HEIGHT//3, RED, center=True, shadow=True)
...
def draw_height_indicator_hud(surface, current_player_height, target_h, vertical_speed_val):
    ...
    vsi_color = VSI_CLIMB_COLOR if vertical_speed_val > 0.05 else (VSI_SINK_COLOR if vertical_speed_val < -0.05 else WHITE)
    ...
    draw_text(surface, f"{int(current_player_height)}m", 14, vsi_text_x -5, player_height_text_y, INDICATOR_PLAYER_COLOR)
...
def draw_weather_vane(surface, wind_x, wind_y, center_x, center_y, max_strength_for_scaling=MAX_WIND_STRENGTH):
    ...
    vane_circle_radius = 3; vane_circle_color = GRAY; strength_ratio = 0.0
...
while running:
    ...
    if game_state == STATE_START_SCREEN:
        ...
        draw_text(screen, f"Target: {TARGET_HEIGHT_PER_LEVEL}m", 20, hud_margin + 120, current_y_hud, text_color_on_hud); current_y_hud += line_spacing
        timer_seconds = (current_ticks - level_timer_start_ticks) / 1000.0
        draw_text(screen, f"Time: {timer_seconds:.1f}s", 20, hud_margin, current_y_hud, text_color_on_hud); current_y_hud += line_spacing
