...
AI_TARGET_SPEED_UPDATE_INTERVAL = 120 # Frames (e.g., 120 frames = 2 seconds at 60FPS)
AI_SPEED_VARIATION_FACTOR = 0.2 # +/- 20% of (max-min speed) for random variation
...
LAND_TYPE_COLORS = {
    LAND_TYPE_WATER_DEEP: PASTEL_WATER_DEEP, LAND_TYPE_WATER_SHALLOW: PASTEL_WATER_SHALLOW, LAND_TYPE_PLAINS: PASTEL_PLAINS, LAND_TYPE_GRASSLAND: PASTEL_GRASSLAND,
    LAND_TYPE_FOREST_TEMPERATE: PASTEL_FOREST_TEMPERATE, LAND_TYPE_FOREST_DENSE: PASTEL_FOREST_DENSE, LAND_TYPE_MOUNTAIN_BASE: PASTEL_MOUNTAIN_BASE, 
    LAND_TYPE_MOUNTAIN_PEAK: PASTEL_MOUNTAIN_PEAK, LAND_TYPE_SAND_DESERT: PASTEL_SAND_DESERT, LAND_TYPE_SAND_BEACH: PASTEL_SAND_BEACH, LAND_TYPE_RIVER: PASTEL_RIVER,
}
LAND_TYPE_THERMAL_PROBABILITY = {
    LAND_TYPE_WATER_DEEP: 0.00, LAND_TYPE_WATER_SHALLOW: 0.01, LAND_TYPE_PLAINS: 0.6, LAND_TYPE_GRASSLAND: 0.7, LAND_TYPE_FOREST_TEMPERATE: 0.3, 
    LAND_TYPE_FOREST_DENSE: 0.1, LAND_TYPE_MOUNTAIN_BASE: 0.5, LAND_TYPE_MOUNTAIN_PEAK: 0.05, LAND_TYPE_SAND_DESERT: 0.9, LAND_TYPE_SAND_BEACH: 0.8, LAND_TYPE_RIVER: 0.02,
}
...
class GliderBase(pygame.sprite.Sprite):
    def __init__(self, body_color, wing_color, start_world_x=0.0, start_world_y=0.0):
        super().__init__()
        self.fuselage_length = 45; self.fuselage_thickness = 4; self.wing_span = 70; self.wing_chord = 5
        ...
        wing_x_pos = (self.fuselage_length - self.wing_chord) * 0.65; wing_y_pos = (canvas_height - self.wing_span) / 2 
        pygame.draw.rect(self.original_image, wing_color, (wing_x_pos, wing_y_pos, self.wing_chord, self.wing_span))
        tail_plane_x_pos = 0; tail_plane_y_top = (canvas_height - self.tail_plane_span) / 2
        ...
        fin_base_y_center = fuselage_y_top + self.fuselage_thickness / 2; fin_bottom_y = fin_base_y_center - self.fuselage_thickness / 2; fin_tip_y = fin_bottom_y - self.tail_fin_height
        fin_leading_edge_x = tail_plane_x_pos + self.tail_plane_chord * 0.2; fin_trailing_edge_x = tail_plane_x_pos + self.tail_plane_chord * 0.8; fin_tip_x = tail_plane_x_pos + self.tail_plane_chord * 0.5
        pygame.draw.polygon(self.original_image, body_color, [(fin_leading_edge_x, fin_bottom_y), (fin_trailing_edge_x, fin_bottom_y), (fin_tip_x, fin_tip_y)])
        self.image = self.original_image; self.rect = self.image.get_rect(); self.collision_radius = GLIDER_COLLISION_RADIUS
        self.world_x = start_world_x; self.world_y = start_world_y; self.heading = 0; self.bank_angle = 0; self.height = INITIAL_HEIGHT; self.speed = INITIAL_SPEED 
        self.trail_points = []; self.contrail_frame_counter = 0; self.current_target_marker_index = 0; self.laps_completed = 0
    ...
    def update_sprite_rotation_and_position(self, cam_x=None, cam_y=None):
        ...
        if isinstance(self, PlayerGlider): self.rect = self.image.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
        elif cam_x is not None and cam_y is not None: self.rect = self.image.get_rect(center=(self.world_x - cam_x, self.world_y - cam_y))

    def update_contrail(self):
        heading_rad = math.radians(self.heading); self.contrail_frame_counter +=1
        if self.contrail_frame_counter >= CONTRAIL_POINT_DELAY:
            self.contrail_frame_counter = 0; effective_tail_offset = (self.fuselage_length / 2) - 2 
            tail_offset_x_world = -effective_tail_offset * math.cos(heading_rad); tail_offset_y_world = -effective_tail_offset * math.sin(heading_rad)
    ...
    def draw_contrail(self, surface, cam_x, cam_y):
        if len(self.trail_points) > 1:
            for i, world_point in enumerate(self.trail_points):
                alpha = int(200 * (i / CONTRAIL_LENGTH)); screen_px = world_point[0] - cam_x; screen_py = world_point[1] - cam_y
                if 0 <= screen_px <= SCREEN_WIDTH and 0 <= screen_py <= SCREEN_HEIGHT:
                    try: pygame.draw.circle(surface, (*PASTEL_CONTRAIL_COLOR, alpha), (screen_px, screen_py), 2)
                    except pygame.error: pass

    def apply_collision_effect(self):
        self.speed *= 0.5; knockback_angle = random.uniform(0, 2 * math.pi); knockback_dist = 5 
        self.world_x += knockback_dist * math.cos(knockback_angle); self.world_y += knockback_dist * math.sin(knockback_angle)

# --- PlayerGlider Class ---
class PlayerGlider(GliderBase):
    def __init__(self):
        super().__init__(PASTEL_GLIDER_BODY, PASTEL_GLIDER_WING)
        self.rect = self.image.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2)); self.previous_height = INITIAL_HEIGHT; self.vertical_speed = 0.0 

    def reset(self, start_height=INITIAL_HEIGHT):
        self.world_x = 0.0; self.world_y = 0.0; self.rect.center = (SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2); self.heading = 0; self.bank_angle = 0
        self.height = start_height; self.speed = INITIAL_SPEED; self.previous_height = start_height; self.vertical_speed = 0.0
        self.trail_points = []; self.contrail_frame_counter = 0; self.current_target_marker_index = 0; self.laps_completed = 0; self.update_sprite_rotation_and_position() 

    def update(self, keys):
        global current_wind_speed_x, current_wind_speed_y, game_state, race_course_markers, total_race_laps, time_taken_for_level, level_timer_start_ticks, game_difficulty
        self.previous_height = self.height
        if keys[pygame.K_UP]: self.speed += ACCELERATION
        elif keys[pygame.K_DOWN]: 
            potential_new_speed = self.speed - ACCELERATION
            if potential_new_speed >= MIN_SPEED: self.speed = potential_new_speed; self.height += ACCELERATION * ZOOM_CLIMB_FACTOR 
        ...
        if game_difficulty == DIFFICULTY_NOOB: actual_turn_rate_factor *= NOOB_TURN_FACTOR
        elif game_difficulty == DIFFICULTY_EASY: actual_turn_rate_factor *= EASY_TURN_FACTOR
        else: actual_turn_rate_factor *= NORMAL_TURN_FACTOR
        turn_rate_degrees = self.bank_angle * actual_turn_rate_factor; self.heading = (self.heading + turn_rate_degrees) % 360
        heading_rad = math.radians(self.heading)
        self.world_x += self.speed * math.cos(heading_rad) + current_wind_speed_x; self.world_y += self.speed * math.sin(heading_rad) + current_wind_speed_y
        height_change_due_to_physics = -GRAVITY_BASE_PULL - STALL_SINK_PENALTY if self.speed < STALL_SPEED else max((self.speed*LIFT_PER_SPEED_UNIT)-GRAVITY_BASE_PULL, -MINIMUM_SINK_RATE) if (self.speed*LIFT_PER_SPEED_UNIT)-GRAVITY_BASE_PULL < 0 else (self.speed*LIFT_PER_SPEED_UNIT)-GRAVITY_BASE_PULL
        self.height += height_change_due_to_physics
        if height_change_due_to_physics < 0: self.speed = min(self.speed + abs(height_change_due_to_physics) * DIVE_TO_SPEED_FACTOR, MAX_SPEED)
        self.vertical_speed = self.height - self.previous_height; self.update_sprite_rotation_and_position(); self.update_contrail()
            ...
            if math.hypot(self.world_x - target_marker.world_pos.x, self.world_y - target_marker.world_pos.y) < target_marker.world_radius: 
                ...
                if self.current_target_marker_index >= len(race_course_markers): 
                    ...
                    if self.laps_completed >= total_race_laps:
                        game_state = STATE_RACE_COMPLETE; time_taken_for_level = (pygame.time.get_ticks() - level_timer_start_ticks) / 1000.0 
    ...
    def apply_lift_from_thermal(self, thermal_lift_power_at_nominal_speed):
        ...
        if game_difficulty == DIFFICULTY_EASY: actual_lift_power *= EASY_MODE_THERMAL_LIFT_MULTIPLIER
        elif game_difficulty == DIFFICULTY_NOOB: actual_lift_power *= NOOB_MODE_THERMAL_LIFT_MULTIPLIER
...
class AIGlider(GliderBase):
    def __init__(self, start_world_x, start_world_y):
        ...
        self.speed_update_timer = 0 # Timer for random speed changes
    ...
    def update(self, cam_x, cam_y, race_markers_list, total_laps_in_race):
        ...
        dx = target_marker.world_pos.x - self.world_x; dy = target_marker.world_pos.y - self.world_y
        dist_to_marker = math.hypot(dx, dy)
        
        target_angle_rad = math.atan2(dy, dx); target_angle_deg = math.degrees(target_angle_rad)
        current_heading_deg = self.heading % 360; target_angle_deg %= 360
        ...
        if dist_to_marker < AI_MARKER_APPROACH_SLOWDOWN_DISTANCE:
            ...
            self.speed_update_timer = 0 # Reset timer when approaching marker
        else:
            # Periodically change target speed if not approaching a marker
            ...
            if self.speed_update_timer >= AI_TARGET_SPEED_UPDATE_INTERVAL:
                ...
                self.target_speed = self.speed + random_variation # Vary based on current speed
                self.target_speed = max(AI_SPEED_MIN, min(self.target_speed, AI_SPEED_MAX)) # Clamp
        ...
        alt_diff = AI_TARGET_RACE_ALTITUDE - self.height; self.height += alt_diff * AI_ALTITUDE_CORRECTION_RATE 
        if self.height < 0: self.height = 0 
        
        heading_rad = math.radians(self.heading)
        self.world_x += self.speed * math.cos(heading_rad); self.world_y += self.speed * math.sin(heading_rad)
        ...
        self.update_sprite_rotation_and_position(cam_x, cam_y); self.update_contrail()

# --- Thermal Class, RaceMarker Class, Minimap Class, ForegroundCloud Class ... (code unchanged) ---
# (Omitting unchanged classes for brevity in this explanation, they are in the full code block)
...
class RaceMarker(pygame.sprite.Sprite):
    ...
    def _draw_marker_image(self, is_active):
        ...
        if is_active and self.number == 1: color_to_use = PASTEL_GOLD
        self.image.fill((0,0,0,0)) 
...
class Minimap:
    def __init__(self, width, height, margin):
        self.width = width; self.height = height; self.margin = margin
    ...
    def world_to_minimap(self, world_x, world_y, player_world_x, player_world_y):
        ...
        rel_x = world_x - player_world_x; rel_y = world_y - player_world_y
        mini_x = self.width / 2 + rel_x * scale; mini_y = self.height / 2 + rel_y * scale
    ...
    def draw(self, surface, player_glider, ai_gliders_list, course_markers):
        ...
        for i, marker in enumerate(course_markers):
            ...
            if 0 <= mini_x <= self.width and 0 <= mini_y <= self.height: 
                ...
                if i == player_glider.current_target_marker_index and marker.number == 1: color = PASTEL_GOLD
...
class ForegroundCloud(pygame.sprite.Sprite):
    def __init__(self, initial_distribution=False, index=0, total_clouds=NUM_FOREGROUND_CLOUDS): 
        ...
        self.x_float = 0.0; self.y_float = 0.0
        if initial_distribution: 
            ...
            if self.dx == 0 and self.dy == 0: 
                if random.choice([True, False]): 
                    self.x_float = float(random.choice([-self.width - 20, SCREEN_WIDTH + 20])); self.y_float = float(random.randint(-self.height, SCREEN_HEIGHT))
                else: 
                    self.y_float = float(random.choice([-self.height - 20, SCREEN_HEIGHT + 20])); self.x_float = float(random.randint(-self.width, SCREEN_WIDTH))
            else: 
                if abs(self.dx) > abs(self.dy): 
                    self.x_float = float(SCREEN_WIDTH + random.randint(0,100) + self.width/2 if self.dx < 0 else -random.randint(0,100) - self.width/2) 
                    self.y_float = float(random.randint(-self.height//2, SCREEN_HEIGHT - self.height//2))
                else: 
                    self.y_float = float(SCREEN_HEIGHT + random.randint(0,50) + self.height/2 if self.dy < 0 else -random.randint(0,50) - self.height/2) 
    ...
    def update(self):
        ...
        off_screen_margin_x = self.width * 1.5 + abs(self.dx * 20); off_screen_margin_y = self.height * 1.5 + abs(self.dy * 20)
        despawn = False
...
NUM_MAJOR_RIVERS = 3; MAJOR_RIVERS_PARAMS = []; _river_param_random = random.Random() 

def regenerate_river_parameters():
    global MAJOR_RIVERS_PARAMS, _river_param_random; MAJOR_RIVERS_PARAMS = []
    for _ in range(NUM_MAJOR_RIVERS):
        ...
        MAJOR_RIVERS_PARAMS.append({
            "amplitude": _river_param_random.uniform(10, 30), "wavelength": _river_param_random.uniform(200, 450), 
            "phase_offset": _river_param_random.uniform(0, 2 * math.pi), "base_x_offset": start_tile_x, 
            "base_y_offset": start_tile_y, "orientation": _river_param_random.choice(['horizontal', 'vertical']),
...
def get_seeded_random_value_direct(unique_tile_x, unique_tile_y, scale, p_pair):
    ...
    scaled_x = math.floor(unique_tile_x / scale); scaled_y = math.floor(unique_tile_y / scale)
...
def get_land_type_at_world_pos(world_x, world_y):
    ...
    if cache_key in tile_type_cache: return tile_type_cache[cache_key]
    ...
    elevation = min(max(math.pow(0.50 * e_continent + 0.35 * e_mountain + 0.15 * e_hill, 1.8), 0.0), 1.0) 
    ...
    moisture = min(max(math.pow(0.7 * m_primary + 0.3 * m_secondary, 1.2), 0.0), 1.0) 
    ...
    if elevation < deep_water_thresh: final_type = LAND_TYPE_WATER_DEEP
    ...
    elif elevation < beach_thresh: final_type = LAND_TYPE_SAND_DESERT if moisture < desert_thresh * 1.2 else LAND_TYPE_SAND_BEACH
        ...
        else: final_type = LAND_TYPE_FOREST_DENSE if moisture > 0.8 and elevation < mountain_base_thresh * 0.9 else LAND_TYPE_FOREST_TEMPERATE
    can_have_river = final_type not in (LAND_TYPE_MOUNTAIN_PEAK, LAND_TYPE_WATER_DEEP) and not (final_type == LAND_TYPE_SAND_DESERT and moisture < desert_thresh * 0.75)
    if can_have_river:
        for params in MAJOR_RIVERS_PARAMS:
            if params["orientation"] == 'horizontal':
                if abs(unique_tile_y - (params["amplitude"] * math.sin((unique_tile_x / params["wavelength"]) * 2 * math.pi + params["phase_offset"]) + params["base_y_offset"])) < params["width"]:
                    final_type = LAND_TYPE_RIVER; break
            else: 
                if abs(unique_tile_x - (params["amplitude"] * math.sin((unique_tile_y / params["wavelength"]) * 2 * math.pi + params["phase_offset"]) + params["base_x_offset"])) < params["width"]:
                    final_type = LAND_TYPE_RIVER; break
    tile_type_cache[cache_key] = final_type; return final_type

def draw_endless_map(surface, cam_x, cam_y):
    start_world_tile_x_coord = math.floor(cam_x / TILE_SIZE) * TILE_SIZE; start_world_tile_y_coord = math.floor(cam_y / TILE_SIZE) * TILE_SIZE
    num_tiles_x = SCREEN_WIDTH // TILE_SIZE + 2; num_tiles_y = SCREEN_HEIGHT // TILE_SIZE + 2
    for i in range(num_tiles_y): 
        for j in range(num_tiles_x): 
            current_tile_world_x = start_world_tile_x_coord + j * TILE_SIZE; current_tile_world_y = start_world_tile_y_coord + i * TILE_SIZE
            tile_screen_x = current_tile_world_x - cam_x; tile_screen_y = current_tile_world_y - cam_y
            ...
            if MAP_TILE_OUTLINE_WIDTH > 0: pygame.draw.rect(surface, MAP_TILE_OUTLINE_COLOR, (tile_screen_x, tile_screen_y, TILE_SIZE, TILE_SIZE), MAP_TILE_OUTLINE_WIDTH)

# --- Text Rendering Helper ---
font_cache = {} 
def get_cached_font(font_name, size):
    key = (font_name, size); 
    if key not in font_cache: font_cache[key] = pygame.font.Font(font_name, size) 
...
def draw_text(surface, text, size, x, y, color=PASTEL_WHITE, font_name=None, center=False, antialias=True, shadow=False, shadow_color=PASTEL_DARK_GRAY, shadow_offset=(1,1)):
    font = get_cached_font(font_name, size); text_surface = font.render(text, antialias, color); text_rect = text_surface.get_rect()
    if center: text_rect.center = (x,y)
    else: text_rect.topleft = (x,y)
    if shadow: surface.blit(font.render(text, antialias, shadow_color), (text_rect.x + shadow_offset[0], text_rect.y + shadow_offset[1]))
    surface.blit(text_surface, text_rect) 

# --- Pygame Setup ---
pygame.init(); pygame.mixer.init(); screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
pygame.display.set_caption("Pastel Glider - Floating Dreams"); clock = pygame.time.Clock()

# --- Game Objects & Variables ---
player = PlayerGlider(); ai_gliders = pygame.sprite.Group() 
all_world_sprites = pygame.sprite.Group(); thermals_group = pygame.sprite.Group(); race_markers_group = pygame.sprite.Group(); foreground_clouds_group = pygame.sprite.Group() 
game_state = STATE_START_SCREEN; current_level = 1; level_timer_start_ticks = 0; time_taken_for_level = 0.0 
current_thermal_spawn_rate = BASE_THERMAL_SPAWN_RATE; thermal_spawn_timer = 0; final_score = 0 
selected_difficulty_option = DIFFICULTY_NORMAL; selected_mode_option = MODE_FREE_FLY; selected_laps_option = 1; lap_options = [1, 3, 5]                      
minimap = Minimap(MINIMAP_WIDTH, MINIMAP_HEIGHT, MINIMAP_MARGIN)

def generate_race_course(num_markers=8):
    global race_course_markers, race_markers_group, all_world_sprites; race_course_markers.clear() 
    for sprite in list(all_world_sprites): 
        if isinstance(sprite, RaceMarker): sprite.kill() 
    for i in range(num_markers):
        marker = RaceMarker(random.uniform(-RACE_COURSE_AREA_HALFWIDTH, RACE_COURSE_AREA_HALFWIDTH), random.uniform(-RACE_COURSE_AREA_HALFWIDTH, RACE_COURSE_AREA_HALFWIDTH), i + 1) 
        race_course_markers.append(marker); all_world_sprites.add(marker)      

def generate_new_wind():
    global current_wind_speed_x, current_wind_speed_y
    wind_angle_rad = random.uniform(0, 2 * math.pi); wind_strength = random.uniform(0.05, MAX_WIND_STRENGTH) 
    current_wind_speed_x = wind_strength * math.cos(wind_angle_rad); current_wind_speed_y = wind_strength * math.sin(wind_angle_rad)

def start_new_level(level_param): 
    global current_level,level_timer_start_ticks,current_thermal_spawn_rate,thermal_spawn_timer, game_state, current_map_offset_x, current_map_offset_y, _river_param_random, total_race_laps, ai_gliders, tile_type_cache
    level_timer_start_ticks = pygame.time.get_ticks() 
    current_map_offset_x = random.randint(-200000, 200000); current_map_offset_y = random.randint(-200000, 200000); tile_type_cache.clear() 
    _river_param_random.seed(current_level + pygame.time.get_ticks()); regenerate_river_parameters(); generate_new_wind() 
    thermals_group.empty(); all_world_sprites.empty(); race_course_markers.clear(); ai_gliders.empty(); foreground_clouds_group.empty()
    for i in range(NUM_FOREGROUND_CLOUDS): foreground_clouds_group.add(ForegroundCloud(initial_distribution=True,index=i))
    ...
    if current_game_mode == MODE_FREE_FLY:
        ...
        elif game_difficulty == DIFFICULTY_EASY: current_thermal_spawn_rate = max(30, int(current_thermal_spawn_rate * 0.75))
        thermal_spawn_timer=0; game_state = STATE_PLAYING_FREE_FLY
    elif current_game_mode == MODE_RACE:
        total_race_laps = level_param; generate_race_course() 
        for i in range(NUM_AI_OPPONENTS):
            angle_offset_from_player_heading = math.pi + (i - NUM_AI_OPPONENTS / 2.0) * (math.pi / 6); dist_offset = 70 + i * 30 
            ...
            new_ai = AIGlider(ai_start_x, ai_start_y); ai_gliders.add(new_ai); all_world_sprites.add(new_ai) 
        ...
        thermal_spawn_timer=0; game_state = STATE_RACE_PLAYING

def reset_to_main_menu():
    global game_state,current_level,final_score, current_wind_speed_x, current_wind_speed_y, selected_difficulty_option, selected_mode_option, selected_laps_option, ai_gliders, tile_type_cache
    player.reset(); thermals_group.empty(); all_world_sprites.empty(); race_markers_group.empty(); race_course_markers.clear(); ai_gliders.empty(); foreground_clouds_group.empty(); tile_type_cache.clear() 
    current_wind_speed_x = -0.2; current_wind_speed_y = 0.05
    for i in range(NUM_FOREGROUND_CLOUDS): foreground_clouds_group.add(ForegroundCloud(initial_distribution=True,index=i))
    current_level=1; final_score=0; selected_difficulty_option = DIFFICULTY_NORMAL; selected_mode_option = MODE_FREE_FLY; selected_laps_option = 1; game_state = STATE_START_SCREEN
...
def draw_difficulty_select_screen(surface, selected_option_idx): 
    surface.fill(PASTEL_DARK_GRAY); draw_text(surface, "Select Difficulty", 56, SCREEN_WIDTH//2, SCREEN_HEIGHT//5, PASTEL_GOLD, center=True, shadow=True, shadow_color=PASTEL_BLACK)
    option_spacing = 100; start_y = SCREEN_HEIGHT//2 - option_spacing
    difficulties_display = [("N00b", "(Largest, Longest, Strongest Thermals!)", DIFFICULTY_NOOB), ("Easy", "(Stronger Thermals)", DIFFICULTY_EASY), ("Normal", "(Standard Challenge)", DIFFICULTY_NORMAL)]
...
def draw_mode_select_screen(surface, selected_option_idx): 
    surface.fill(PASTEL_DARK_GRAY); draw_text(surface, "Select Mode", 56, SCREEN_WIDTH//2, SCREEN_HEIGHT//4, PASTEL_GOLD, center=True, shadow=True, shadow_color=PASTEL_BLACK)
    modes_display = [("Free Fly", "(Explore & Reach Altitude Goals)", MODE_FREE_FLY), ("Race", "(Fly Through Markers Against AI)", MODE_RACE)]
...
def draw_laps_select_screen(surface, selected_lap_idx, lap_choices_list): 
    surface.fill(PASTEL_DARK_GRAY); draw_text(surface, "Select Laps", 56, SCREEN_WIDTH//2, SCREEN_HEIGHT//4, PASTEL_GOLD, center=True, shadow=True, shadow_color=PASTEL_BLACK)
...
def draw_target_reached_options_screen(surface, level, time_taken_seconds_val):
    surface.fill(PASTEL_DARK_GRAY); draw_text(surface, f"Level {level} Goal Reached!", 60, SCREEN_WIDTH//2, SCREEN_HEIGHT//3 - 20, PASTEL_GOLD, center=True, shadow=True, shadow_color=PASTEL_BLACK)
...
def draw_post_goal_menu_screen(surface, level): 
    surface.fill(PASTEL_DARK_GRAY); draw_text(surface, f"Level {level} - Cruising", 50, SCREEN_WIDTH//2, SCREEN_HEIGHT//4, PASTEL_GOLD, center=True, shadow=True, shadow_color=PASTEL_BLACK)
...
def draw_race_complete_screen(surface, total_time_seconds):
    surface.fill(PASTEL_DARK_GRAY); draw_text(surface, "Race Finished!", 60, SCREEN_WIDTH//2, SCREEN_HEIGHT//3 -20, PASTEL_GOLD, center=True, shadow=True, shadow_color=PASTEL_BLACK)
...
def draw_game_over_screen_content(surface, final_player_height, level_reached):
    ...
    if current_game_mode == MODE_FREE_FLY: draw_text(surface, f"Reached Level: {level_reached}", 36, SCREEN_WIDTH//2, SCREEN_HEIGHT//2-30, PASTEL_WHITE, center=True)
...
def draw_height_indicator_hud(surface, current_player_height, target_h_for_level, vertical_speed_val):
    indicator_bar_height = SCREEN_HEIGHT - HUD_HEIGHT - (2 * INDICATOR_Y_MARGIN_FROM_HUD); indicator_x_pos = SCREEN_WIDTH - INDICATOR_WIDTH - INDICATOR_X_MARGIN; indicator_y_pos = HUD_HEIGHT + INDICATOR_Y_MARGIN_FROM_HUD 
    ...
    if current_player_height > 0: player_marker_y_on_bar = indicator_y_pos + indicator_bar_height * (1 - min(current_player_height / max_indicator_height_value, 1.0))
    ...
    vsi_text_x = indicator_x_pos - 70; vsi_arrow_x_center = indicator_x_pos - 10 
    ...
    if abs(vsi_mps) > 0.5: 
        arrow_points = [ (vsi_arrow_x_center, player_marker_y_on_bar - VSI_ARROW_SIZE if vsi_mps > 0 else player_marker_y_on_bar + VSI_ARROW_SIZE), 
                         (vsi_arrow_x_center - VSI_ARROW_SIZE // 2, player_marker_y_on_bar), 
                         (vsi_arrow_x_center + VSI_ARROW_SIZE // 2, player_marker_y_on_bar)] 
        if vsi_mps < 0: arrow_points.reverse() # Correct arrow direction for sinking
        pygame.draw.polygon(surface, vsi_color, arrow_points)
    player_height_text_y = player_marker_y_on_bar - 20 
    if player_height_text_y < indicator_y_pos + 5 : player_height_text_y = player_marker_y_on_bar + 15 
...
def draw_weather_vane(surface, wind_x, wind_y, center_x, center_y, max_strength_for_scaling=MAX_WIND_STRENGTH):
    vane_angle_rad = math.atan2(wind_y, wind_x) + math.pi; wind_magnitude = math.hypot(wind_x, wind_y)
    arrow_base_length = 15; arrow_max_additional_length = 20; arrow_color = PASTEL_TEXT_COLOR_HUD; arrow_thickness = 2
    vane_circle_radius = 3; vane_circle_color = PASTEL_GRAY
    ...
    tip_x = center_x + current_arrow_length * 0.7 * math.cos(vane_angle_rad); tip_y = center_y + current_arrow_length * 0.7 * math.sin(vane_angle_rad)
    tail_x = center_x - current_arrow_length * 0.3 * math.cos(vane_angle_rad); tail_y = center_y - current_arrow_length * 0.3 * math.sin(vane_angle_rad) 
    pygame.draw.line(surface, arrow_color, (tail_x, tail_y), (tip_x, tip_y), arrow_thickness)
    barb_length = 8; barb_angle_offset = math.radians(150) 
...
running = True
while running:
    dt = clock.tick(60) / 1000.0; current_ticks = pygame.time.get_ticks(); keys = pygame.key.get_pressed()
    for event in pygame.event.get():
        if event.type == pygame.QUIT: running = False
        if event.type == pygame.KEYDOWN:
            if game_state == STATE_START_SCREEN and event.key == pygame.K_RETURN: game_state = STATE_DIFFICULTY_SELECT
                ...
                elif event.key == pygame.K_RETURN: game_difficulty = selected_difficulty_option; game_state = STATE_MODE_SELECT
                ...
                if event.key == pygame.K_UP or event.key == pygame.K_DOWN: selected_mode_option = 1 - selected_mode_option
                    ...
                    if current_game_mode == MODE_FREE_FLY: start_new_level(1) 
                    else: game_state = STATE_RACE_LAPS_SELECT
                ...
                elif event.key == pygame.K_RETURN: start_new_level(lap_options[selected_laps_option]) 
            elif game_state == STATE_PLAYING_FREE_FLY and event.key == pygame.K_ESCAPE: reset_to_main_menu()
            elif game_state == STATE_TARGET_REACHED_OPTIONS: 
                if event.key == pygame.K_m: start_new_level(current_level + 1)
                elif event.key == pygame.K_c: game_state = STATE_TARGET_REACHED_CONTINUE_PLAYING
            elif game_state == STATE_TARGET_REACHED_CONTINUE_PLAYING and event.key == pygame.K_ESCAPE: game_state = STATE_POST_GOAL_MENU
            elif game_state == STATE_POST_GOAL_MENU: 
                if event.key == pygame.K_n: start_new_level(current_level + 1)
                elif event.key == pygame.K_q: reset_to_main_menu()
                elif event.key == pygame.K_r or event.key == pygame.K_ESCAPE : game_state = STATE_TARGET_REACHED_CONTINUE_PLAYING
            elif game_state == STATE_RACE_PLAYING and event.key == pygame.K_ESCAPE: reset_to_main_menu()
            elif game_state == STATE_RACE_COMPLETE and event.key == pygame.K_RETURN: reset_to_main_menu()
            elif game_state == STATE_GAME_OVER and event.key == pygame.K_RETURN: reset_to_main_menu()

    if game_state in (STATE_PLAYING_FREE_FLY, STATE_TARGET_REACHED_CONTINUE_PLAYING, STATE_RACE_PLAYING):
        player.update(keys); camera_x = player.world_x - SCREEN_WIDTH // 2; camera_y = player.world_y - SCREEN_HEIGHT // 2
        if game_state == STATE_RACE_PLAYING:
            for ai in ai_gliders: ai.update(camera_x, camera_y, race_course_markers, total_race_laps)
            ...
            for i in range(len(ai_list_for_collision)):
                for j in range(i + 1, len(ai_list_for_collision)):
                    ai1, ai2 = ai_list_for_collision[i], ai_list_for_collision[j]
                    if ((ai1.world_x - ai2.world_x)**2 + (ai1.world_y - ai2.world_y)**2) < (ai1.collision_radius + ai2.collision_radius)**2:
                        ai1.apply_collision_effect(); ai2.apply_collision_effect()
        ...
        if thermal_spawn_timer >= current_thermal_spawn_rate:
            thermal_spawn_timer = 0; spawn_world_x = camera_x + random.randint(-THERMAL_SPAWN_AREA_WIDTH // 2, THERMAL_SPAWN_AREA_WIDTH // 2); spawn_world_y = camera_y + random.randint(-THERMAL_SPAWN_AREA_HEIGHT // 2, THERMAL_SPAWN_AREA_HEIGHT // 2)
            if random.random() < LAND_TYPE_THERMAL_PROBABILITY.get(get_land_type_at_world_pos(spawn_world_x, spawn_world_y), 0.0):
                new_thermal = Thermal((spawn_world_x, spawn_world_y)); all_world_sprites.add(new_thermal); thermals_group.add(new_thermal)
        if game_state == STATE_RACE_PLAYING:
            for i, marker in enumerate(race_course_markers): marker.update(camera_x, camera_y, i == player.current_target_marker_index)
        ...
        for thermal in thermals_group:
            if math.hypot(player.world_x - thermal.world_pos.x, player.world_y - thermal.world_pos.y) < thermal.radius + (player.collision_radius * 0.5): player.apply_lift_from_thermal(thermal.lift_power)
        if game_state == STATE_PLAYING_FREE_FLY and player.height >= TARGET_HEIGHT_PER_LEVEL * current_level:
            game_state = STATE_TARGET_REACHED_OPTIONS; level_end_ticks = pygame.time.get_ticks(); time_taken_for_level = (level_end_ticks - level_timer_start_ticks) / 1000.0
        if player.height <= 0: final_score = player.height; player.height = 0; game_state = STATE_GAME_OVER

    screen.fill(PASTEL_BLACK) 
    if game_state == STATE_START_SCREEN: draw_start_screen_content(screen); foreground_clouds_group.draw(screen) 
    elif game_state == STATE_DIFFICULTY_SELECT: draw_difficulty_select_screen(screen, selected_difficulty_option); foreground_clouds_group.draw(screen)
    elif game_state == STATE_MODE_SELECT: draw_mode_select_screen(screen, selected_mode_option); foreground_clouds_group.draw(screen)
    elif game_state == STATE_RACE_LAPS_SELECT: draw_laps_select_screen(screen, selected_laps_option, lap_options); foreground_clouds_group.draw(screen)
    elif game_state == STATE_TARGET_REACHED_OPTIONS: draw_target_reached_options_screen(screen, current_level, time_taken_for_level); foreground_clouds_group.draw(screen)
    elif game_state == STATE_POST_GOAL_MENU: draw_post_goal_menu_screen(screen, current_level); foreground_clouds_group.draw(screen)
    elif game_state == STATE_RACE_COMPLETE: draw_race_complete_screen(screen, time_taken_for_level); foreground_clouds_group.draw(screen)
        ...
        for ai_glider_sprite in ai_gliders: ai_glider_sprite.draw_contrail(screen, camera_x, camera_y)
        all_world_sprites.draw(screen); screen.blit(player.image, player.rect) 
        ...
        if current_game_mode == MODE_FREE_FLY:
            ...
            if race_course_markers and player.current_target_marker_index < len(race_course_markers) and game_state == STATE_RACE_PLAYING :
                ...
                draw_text(screen, f"Marker {target_marker.number}: {int(math.hypot(player.world_x - target_marker.world_pos.x, player.world_y - target_marker.world_pos.y)/10.0)} u", 20, hud_margin + 150, current_y_hud, PASTEL_TEXT_COLOR_HUD)
                if target_marker.rect.right > 0 and target_marker.rect.left < SCREEN_WIDTH and target_marker.rect.bottom > 0 and target_marker.rect.top < SCREEN_HEIGHT:
                    marker_screen_x = target_marker.rect.centerx; marker_screen_y = target_marker.rect.centery
                    hud_arrow_base_x = SCREEN_WIDTH - MINIMAP_WIDTH - MINIMAP_MARGIN - 50; hud_arrow_base_y = MINIMAP_MARGIN + HUD_HEIGHT + MINIMAP_HEIGHT // 2
                    angle_to_marker_screen = math.atan2(marker_screen_y - hud_arrow_base_y, marker_screen_x - hud_arrow_base_x)
                    arrow_len = 20; tip_x = hud_arrow_base_x + arrow_len * math.cos(angle_to_marker_screen); tip_y = hud_arrow_base_y + arrow_len * math.sin(angle_to_marker_screen)
                    p1 = (tip_x, tip_y); p2 = (tip_x - arrow_len * 0.5 * math.cos(angle_to_marker_screen + math.radians(150)), tip_y - arrow_len * 0.5 * math.sin(angle_to_marker_screen + math.radians(150)))
                    p3 = (tip_x - arrow_len * 0.5 * math.cos(angle_to_marker_screen - math.radians(150)), tip_y - arrow_len * 0.5 * math.sin(angle_to_marker_screen - math.radians(150)))
                    pygame.draw.polygon(screen, PASTEL_ACTIVE_MARKER_COLOR, [p1,p2,p3])
        current_y_hud += line_spacing
        timer_seconds = (current_ticks - level_timer_start_ticks) / 1000.0 if game_state == STATE_PLAYING_FREE_FLY or game_state == STATE_RACE_PLAYING else time_taken_for_level
        draw_text(screen, f"Time: {timer_seconds:.1f}s{' (Goal!)' if game_state == STATE_TARGET_REACHED_CONTINUE_PLAYING else ''}", 20, hud_margin, current_y_hud, PASTEL_TEXT_COLOR_HUD)
        current_y_hud += line_spacing
        ...
        wind_text_x_pos = SCREEN_WIDTH - MINIMAP_WIDTH - MINIMAP_MARGIN * 2 - 200 
        draw_text(screen, f"Wind: <{current_wind_speed_x*10:.0f}, {current_wind_speed_y*10:.0f}>", 18, wind_text_x_pos, hud_margin + 5, PASTEL_TEXT_COLOR_HUD)
        esc_text = "ESC for Menu" if game_state != STATE_TARGET_REACHED_CONTINUE_PLAYING else "ESC for Options"
        draw_text(screen, esc_text, 18, wind_text_x_pos, hud_margin + 5 + line_spacing, PASTEL_TEXT_COLOR_HUD)
        draw_weather_vane(screen, current_wind_speed_x, current_wind_speed_y, wind_text_x_pos + 170, hud_margin + 25) 
        ...
        if current_game_mode == MODE_RACE and (game_state == STATE_RACE_PLAYING or game_state == STATE_RACE_COMPLETE): minimap.draw(screen, player, ai_gliders, race_course_markers)
    elif game_state == STATE_GAME_OVER: draw_game_over_screen_content(screen, final_score, current_level); foreground_clouds_group.draw(screen)
    pygame.display.flip() 
pygame.quit()
