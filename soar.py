...
class Glider(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.in_thermal_lift = 0 # Tracks lift gained from current thermal interaction this frame
    ...
    def update(self, keys):
        self.in_thermal_lift = 0 # Reset thermal lift for this frame
        ...
        # --- Turning Logic ---
        # Faster speed can mean wider turns unless bank is increased, or keep it simple
        turn_rate_degrees = self.bank_angle * TURN_RATE_SCALAR # Basic turning
        # turn_rate_degrees = self.bank_angle * TURN_RATE_SCALAR * (INITIAL_SPEED / max(self.speed, 0.1)) # Speed affects turning radius
        ...
        # Lift generated by speed
        lift_from_speed = self.speed * LIFT_PER_SPEED_UNIT
        # Net vertical change due to gravity and speed-based lift
        natural_height_change = lift_from_speed - GRAVITY_BASE_PULL
        
        # Apply net change, ensuring a minimum sink rate if not in thermal
        # If self.in_thermal_lift is positive, it will be added later
        if self.in_thermal_lift <= 0: # Only apply natural sink/lift if not actively in strong thermal lift
             self.height += max(natural_height_change, -MINIMUM_SINK_RATE if natural_height_change < 0 else natural_height_change)
        # The thermal lift will be added separately if a thermal is hit this frame
    ...
    def apply_thermal_lift(self, thermal_base_strength):
        ...
        # The factor is inverse to speed: higher speed means less lift *per frame of contact*
        # We can use a divisor based on speed. Example: if speed is MIN_SPEED, divisor is 1.
        # If speed is MAX_SPEED, divisor is MAX_SPEED/MIN_SPEED.
        # Let's try scaling the lift directly by the inverse of current speed relative to a reference (e.g., initial speed).
        # Lift per frame = base_strength * (reference_speed / current_speed)
        # Ensure current_speed is not zero.
        
        # Simpler: Lift gained is stronger if speed is low.
        # Max lift at MIN_SPEED, min lift (but still positive) at MAX_SPEED.
        # We want the lift to be highest at MIN_SPEED.
        # Lift factor = 1 + ( (INITIAL_SPEED - self.speed) / INITIAL_SPEED ) * some_multiplier
        # This can go negative if speed > INITIAL_SPEED. Let's rethink.

        # More direct: Scale lift inversely with current speed.
        # Higher values when speed is low.
        lift_this_frame = thermal_base_strength * (INITIAL_SPEED / max(self.speed, MIN_SPEED * 0.5)) # Ensure no division by zero or tiny speed
        lift_this_frame = max(lift_this_frame, thermal_base_strength * 0.2) # Ensure some minimum lift even at high speed

        self.height += lift_this_frame
        self.in_thermal_lift += lift_this_frame # Track lift from thermals this frame
...
while running:
    ...
    if game_state == STATE_PLAYING:
        ...
        player.update(keys) # Updates position, speed, heading, and applies natural sink/lift
        ...
        hit_thermals = pygame.sprite.spritecollide(player, thermals_group, True) # dokill=True
        for _ in hit_thermals:
            player.apply_thermal_lift(BASE_THERMAL_STRENGTH)
            # The actual height gain is now handled inside apply_thermal_lift based on speed

        # Check for Game Over (after all height changes, including thermals)
        if player.height <= 0:
            final_score = player.height # Store the exact value (can be negative)
            player.height = 0 # Don't display negative height in UI during game over
